...
/// <summary>
/// Checkpoint loader for SAM3 that supports BOTH safetensors and .pt formats.
///
/// Key format differences:
/// - safetensors: detector_model.vision_encoder.backbone.*, detector_model.text_encoder.text_model.*
/// - pt (HF):     detector.backbone.vision_backbone.*, detector.backbone.language_backbone.*
///
/// For .pt files, delegates loading to a Python helper script that converts tensors to JSON.
/// </summary>
public class Sam3CheckpointLoaderUniversal
{
    ...
    /// <summary>
    /// Load a SAM3 model from either .safetensors or .pt checkpoint file.
    /// </summary>
        ...
        Device device = null)
    {
        ...
        Console.WriteLine($"[CheckpointLoader] Detected format: {format}");
        ...
        {
            checkpoint = LoadPtCheckpointViaPython(checkpointPath, dev);
        }
        ...
        foreach (var kvp in checkpoint)
        {
            ...
            // FPN scale_layers weights are transposed in checkpoint
    ...
    private CheckpointFormat DetectFormat(string path)
    {
        if (path.EndsWith(".safetensors", StringComparison.OrdinalIgnoreCase))
            return CheckpointFormat.Safetensors;

        // For .pt files, check via Python
        var script = $"import torch; sd=torch.load(r'{path.Replace('\\','/').Replace("'", "\\'")}', map_location='cpu', weights_only=False); " +
                     $"d=sd['model'] if isinstance(sd.get('model'), dict) else sd; " +
                     $"print(list(d.keys())[0][:20] if d else 'NONE')";
        ...
        try
        {
            ...
            if (output.StartsWith("detector_model."))
                return CheckpointFormat.Safetensors;
        }
        catch { }

        return CheckpointFormat.PtHuggingFace;
    }

    private Dictionary<string, Tensor> LoadPtCheckpointViaPython(string path, Device device)
    {
        // Write a Python helper script to convert .pt to JSON
        var tempScript = Path.GetTempFileName() + ".py";
        var tempJson = Path.GetTempFileName() + ".json";

        var escapedPath = path.Replace("\\", "\\\\").Replace("\"", "\\\"");
        var escapedOut = tempJson.Replace("\\", "\\\\").Replace("\"", "\\\"");
...
for k, v in sd.items():
    if isinstance(v, torch.Tensor):
        result[k] = {{
            ...
            'data': v.cpu().numpy().tobytes().hex()
        }}
with open(r'{tempJson}', 'w') as f:
    json.dump(result, f)
print(f'Loaded {{len(result)}} tensors')
";
        ...
        try
        {
            ...
            if (proc.ExitCode != 0)
            {
                Console.WriteLine($"[LoadPtCheckpoint] Python error: {err}");
                return new Dictionary<string, Tensor>();
            }

            var jsonText = File.ReadAllText(tempJson);
            var dict = System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, System.Text.Json.JsonElement>>(jsonText);
            ...
            foreach (var kvp in dict)
            {
                var key = kvp.Key;
                var el = kvp.Value;
                var shape = el.GetProperty("shape").EnumerateArray().Select(e => (long)e.GetInt32()).ToArray();
                var dtypeStr = el.GetProperty("dtype").GetString();
                var dataHex = el.GetProperty("data").GetString();

                ScalarType dtype = dtypeStr switch
                {
                    "float32" or "float" => ScalarType.Float32,
                    "float16" or "half" => ScalarType.Float16,
                ...
                var floatArr = new float[bytes.Length / 4];
                System.Buffer.BlockCopy(bytes, 0, floatArr, 0, bytes.Length);
                result[key] = torch.tensor(floatArr, shape, dtype: dtype);
            }
    ...
    {
        if (format == CheckpointFormat.Safetensors)
            return MapSafetensorsKey(ckptKey);
        else
            return MapPtHfKey(ckptKey);
    }
    ...
    {
        if (!ckptKey.StartsWith("detector."))
            return null;
        var rest = ckptKey.Substring("detector.".Length);

        if (rest.StartsWith("tracker.") || rest.StartsWith("sam2_predictor."))
            return null;
        ...
        if (rest.StartsWith("backbone.vision_backbone.trunk."))
        {
            var trunkRest = rest.Substring("backbone.vision_backbone.trunk.".Length);
            return MapHfVitKey(trunkRest);
        }
        ...
        // === TRANSFORMER ===
        if (rest.StartsWith("transformer.encoder.layers."))
        {
            var layerRest = rest.Substring("transformer.encoder.layers.".Length);
            var parts = layerRest.Split('.');
            var idx = parts[0];
            var suffix = string.Join(".", parts.Skip(1));
            return MapTransformerEncoderLayerKey($"detr_encoder.layers.{idx}.{suffix}");
        }
        ...
        if (rest.StartsWith("geometry_encoder."))
        {
            ...
            if (geoRest.StartsWith("cls_embed.") || geoRest.StartsWith("boxes_direct_project.") ||
                ...
                geoRest.StartsWith("vision_layer_norm."))
                return null;
        ...
        {
            if (rest.Contains("semantic_projection") || rest.Contains("instance_projection"))
                return null;
    ...
    private string MapHfVitKey(string trunkRest)
    {
        if (trunkRest.StartsWith("blocks."))
        {
            var blockRest = trunkRest.Substring("blocks.".Length);
        ...
        if (trunkRest == "patch_embed.proj.weight") return "vision_backbone.patch_embed.proj.weight";
        if (trunkRest == "pos_embed") return "vision_backbone.pos_embed_buffer";
        if (trunkRest == "ln_pre.weight") return "vision_backbone.norm.weight";
        if (trunkRest == "ln_pre.bias") return "vision_backbone.norm.bias";
    ...
    private bool TrySetValueByPath(Module module, string path, Tensor value)
    {
        ...
        for (int i = 0; i < parts.Length - 1; i++)
        {
            ...
            {
                if (index < matchingChildren.Count) child = matchingChildren[index].Item2;
            }
        ...
        // Handle fused qkv/in_proj -> split projections
        if (finalKey.EndsWith("_qkv.weight") || finalKey.EndsWith("_qkv.bias"))
        {
            Tensor? fieldTensor = null;
            var fieldType = current.GetType();
            var fieldMember = fieldType.GetField(finalKey.Replace("_qkv", ""), System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic);
            ...
            {
                var d = (int)fieldTensor.size(-1);
                using (var q = value.narrow(-1, 0, (long)d))
                using (var _ng1 = torch.no_grad())
                {
                    fieldTensor.copy_(q.contiguous());
                }
                return true;
            }
        }

        if (finalKey.EndsWith("_in_proj.weight") || finalKey.EndsWith("_in_proj.bias"))
        {
            Tensor? fieldTensor = null;
            var fieldType = current.GetType();
            var fieldMember = fieldType.GetField(finalKey.Replace("_in_proj", ""), System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic);
            if (fieldMember is not null && fieldMember.FieldType == typeof(Tensor))
            {
                fieldTensor = (Tensor)fieldMember.GetValue(current);
            }
        ...
        // Handle regular parameter/buffer/field setting
        Tensor? fieldTensor2 = null;
        var fieldType2 = current.GetType();
        var fieldMember2 = fieldType2.GetField(finalKey, System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic);
        if (fieldMember2 is not null && fieldMember2.FieldType == typeof(Tensor))
        {
            fieldTensor2 = (Tensor)fieldMember2.GetValue(current);
        }
        if (fieldTensor2 is not null)
        {
            if (fieldTensor2.shape.Length == value.shape.Length)
            {
                bool match = true;
                for (int i = 0; i < fieldTensor2.shape.Length; i++)
                {
                    if (fieldTensor2.shape[i] != value.shape[i]) { match = false; break; }
                }
                if (match)
                {
