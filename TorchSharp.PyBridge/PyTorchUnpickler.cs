    ...
    public static class PyTorchUnpickler {
        ...
        class TensorObjectConstructor : IObjectConstructor {
            public object construct(object[] args) {
                ...
                //torch.Tensor t = torch.WrappedTensorDisposeScope(() => torch.empty_strided(shape, stride, arg0.dtype,));
                //if (storageOffset > 0)
                //{
                //    Console.WriteLine($"{storageOffset}\t{string.Join(",", shape)}\t{string.Join(",", stride)}");
                //    storageOffset = 0;
                //}
                //torch.Tensor t = torch.WrappedTensorDisposeScope(() =>
                //                    torch.empty(shape, arg0.dtype).as_strided(shape, stride, storageOffset).contiguous());
                torch.Tensor t = torch.WrappedTensorDisposeScope(() =>
